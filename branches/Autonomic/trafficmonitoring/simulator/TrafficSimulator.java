...
public class TrafficSimulator implements GUI.ControlPanelUser, Runnable{
	...
	public void init(int cameraRange){
		for(Enumeration<RoadSegment> e = this.network.getSegments().elements(); e.hasMoreElements(); ){
	...
	//@Pieter
	public void advance() {
		/*for(Enumeration<RoadSegment> e = this.getSegments(); e.hasMoreElements(); ){
		this.printCars(e.nextElement());
		}*/

		//System.out.println(1);

		for(Enumeration<RoadSegment> e = this.network.getSegments().elements(); e.hasMoreElements(); ){
			e.nextElement().updateSpeeds (this.control.getSlowdown(), this.control.getlambda());
		}

		//System.out.println(2);

		for(Enumeration<Node> e = this.network.getNodes().elements(); e.hasMoreElements(); ){
			e.nextElement().step();
		}
		...
		//System.out.println("In - out: "+ CarCounter.getCounter().getCurrentCars()+"; real cars: "+realCars);
		System.out.println("difference: "+ (CarCounter.getCounter().getCurrentCars() - realCars));
		 */
	}
	...
	public void updateRate(double carRate){
		for(Enumeration<Source> e = this.network.getSources().elements(); e.hasMoreElements(); ){
	...
	public void setNodeFailureScenario(CameraNode[] failingNodes, int[] failTimes){
	...
	private CameraNode[] failingNodes = new CameraNode[0];
...
//		currentStep++;
	...
	public void failNode(int i){
		...
		i = i-1;
		System.out.println("<<<NODE "+failingNodes[i].getComMid().getNodeIdentifier()+" FAILED");
		failingNodes[i].fail();
		SimulatedCommunicationNetwork.getInstance().isolateNode(failingNodes[i].getComMid().getNodeIdentifier());
	}
	
	//@Pieter
	public void bringNodeBackOnline(int i){
		i = i-1;
		System.out.println("<<<NODE "+failingNodes[i].getComMid().getNodeIdentifier()+" BACK ONLINE");
		
		failingNodes[i].bringBackOnline();
		
		SimulatedCommunicationNetwork.getInstance().deIsolateNode(failingNodes[i].getComMid().getNodeIdentifier());
	}
	...
	private void scenario9step(){
		...
		if(currentStep == 301){
			aliveSignals = MessageProfiler.getMessageProfiler().getAmountFor("AliveSignal"); 
		}
		
		if(currentStep == 400){
			aliveSignals = MessageProfiler.getMessageProfiler().getAmountFor("AliveSignal") - aliveSignals;
		}
		...
		if(currentStep>=400+this.failTime){
			if(this.failingNodes[0].getContext().getPersonalOrg() != null){
				int masterID = this.failingNodes[0].getContext().getPersonalOrg().getMasterID().getId();
				if(this.failingNodes[masterID-1].getContext().getPersonalOrg() != null){
					if(this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().size()==5 &&
							!this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().contains(new utilities.NodeID(5))){
								String[] items = {"Alive Signals over interval:", "Nb of steps:"};
								int[] values = {aliveSignals, currentStep-400};
								MessageProfiler.getMessageProfiler().printInOnce(toPrint, true, items, values);
								System.exit(0);
					}
				}
			}
	...
	private void scenario10step(){
		...
		if(currentStep == 400 +this.failTime){
			failedMaster = this.failingNodes[0].getContext().getPersonalOrg().getMasterID();
			this.failNode(this.failingNodes[0].getContext().getPersonalOrg().getMasterID().getId());
			MessageProfiler.getMessageProfiler().reset();
		}
		...
		if(currentStep>=400+this.failTime){
			if(this.failingNodes[0].getContext().getPersonalOrg() != null){
				int masterID = this.failingNodes[0].getContext().getPersonalOrg().getMasterID().getId();
				if(this.failingNodes[masterID-1].getContext().getPersonalOrg() != null){
					if(this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().size()==3 &&
							!this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().contains(failedMaster)){
								String[] items = {"Alive Signals over interval:", "Nb of steps:"};
								int[] values = {aliveSignals, currentStep-400};
								MessageProfiler.getMessageProfiler().printInOnce(toPrint, true, items, values);
								System.exit(0);
	...
	private void scenario11step(){
		...
		if(currentStep>=400+this.failTime){
			if(this.failingNodes[0].getContext().getPersonalOrg() != null){
				...
				if(this.failingNodes[masterID-1].getContext().getPersonalOrg() != null){
					if(this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().size()==3 &&
							!this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().contains(new utilities.NodeID(2))){
								String[] items = {"Alive Signals over interval:", "Nb of steps:"};
								int[] values = {aliveSignals, currentStep-400};
								MessageProfiler.getMessageProfiler().printInOnce(toPrint, true, items, values);
								System.exit(0);
	...
	private void scenario14step(){
		...
		if(currentStep>=400+this.failTime){
			if(this.failingNodes[0].getContext().getPersonalOrg() != null){
				int masterID = this.failingNodes[0].getContext().getPersonalOrg().getMasterID().getId();
				if(this.failingNodes[masterID-1].getContext().getPersonalOrg() != null){
					if(this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().size()==19 &&
							!this.failingNodes[masterID-1].getContext().getPersonalOrg().getAgents().contains(new utilities.NodeID(15))){
								MessageProfiler.getMessageProfiler().printInOnce(toPrint, true);
								System.exit(0);
					}
				}
