...
public class MessageProfiler {
	...
	private static MessageProfiler profiler;
	private static String outputFile;
	
	private MessageProfiler(long startupTime){
		//@Pieter
		
//		try {
			this.register = new Hashtable<String, Integer>();
			this.cache = new Hashtable<String, Integer>();
			this.lastPrintoutTime = startupTime;
//			if(outputFile != null)
//				this.output = new BufferedWriter(new FileWriter(outputFile, true));
//			else this.output = new BufferedWriter(new FileWriter("output.txt", true));
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
	}
	
	public synchronized static MessageProfiler getMessageProfiler(){
		if(profiler == null)
			profiler = new MessageProfiler(System.currentTimeMillis());
		return profiler;
	}
	...
	public void printNodeFailure(String nodeID){
		try {
			this.output.write(System.currentTimeMillis() + "\t" +nodeID + " FAILED");
			this.output.newLine();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void flushResults(int test){
		try{	
			this.output.write("**********************************************************");this.output.newLine();
			this.output.write("START "+test);this.output.newLine();
	...
	public void printInOnce(String[] whatToPrint, boolean printTotal, String[] items, int[] values){
		long intervalLength = System.currentTimeMillis() - lastPrintoutTime;
		try {
			this.output.write(Long.toString(System.currentTimeMillis()) + "\t");
			this.output.write(Long.toString(intervalLength) + "\t");
			...
			for(int i = 0; i<items.length; i++){
				this.output.write(items[i] + "\t"+ values[i] +"\t");
			}
			this.output.newLine();
			this.output.flush();
		} catch (IOException e) {}
	...
	public static void setOutputFile(String of){
		outputFile = of;
	}
	
	public int getAmountFor(String item){
