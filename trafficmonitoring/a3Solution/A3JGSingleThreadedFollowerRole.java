...
import A3JGroups.ElectionManager;

/**
 * A3JGFollowerRole must be extended in order to define a certain type of
 * follower. You must define the behavior of each follower implementing the run
 * function. A followerRole can be used in more than one group, and different
 * role can be used in the same group. In each group there are more follower
 * that work with a supervisor.
 * 
 * @author bett.marco88@gmail.com
 * 
 */

// TODO vedere come si comporta l'election manager

public abstract class A3JGSingleThreadedFollowerRole extends ReceiverAdapter
		implements Runnable {
	...
	public A3JGSingleThreadedFollowerRole(int resourceCost, ThreadManager tm) {
		super();
	...
	public void setNode(A3JGSingleThreadedNode node) {
		this.node = node;
	}

	public A3JGSingleThreadedNode getNode() {
		return node;
	}
	...
	public void receive(Message mex) {
		...
		if (msg.getValueID().equals("A3FitnessFunction")) {
			...
			if (groupName.contains("A3Split"))
				groupName = groupName.substring(groupName
						.lastIndexOf("A3Split"));
			...
			try {
				this.getNode().joinSplitGroup(this.getChan().getClusterName(),
						this.getChan(), port, true);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void viewAccepted(View view) {
		if (!view.getMembers().contains(map.get("A3Supervisor"))
				&& view.getMembers().get(0).equals(chan.getAddress())
				&& attempt < maxAttempt) {
	...
	public boolean sendMessageToSupervisor(A3JGMessage mex) {
		try {
			if (!chan.getView().containsMember(
					(Address) map.get("A3Supervisor")))
				return false;
	...
	/**
	 * This function is used to send an update to the supervisor, using a
	 * message.
	 * 
	 * @param mex
	 *            The A3JGMessage sent to the supervisor.
	...
	/**
	 * This function is used to retrieve messages left in memory on
	 * ReplicatedHashMap.
	 * 
	 * @return The list of A3JGMessage presents on the map.
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * @return True if the change has success, false otherwise.
	 */
	public boolean changeRoleInGroup(int config) {

		String folName = node.getGroupInfo(chan.getClusterName()).getFollower()
				.get(config);
		if (node.getFollowerRole(folName) != null) {
			...
			tm.register(node.getFollowerRole(folName));
