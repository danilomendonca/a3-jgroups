...
import A3JGroups.MessageDelete;
import A3JGroups.SplitManager;

/**
 * A3JGSupervisorRole must be extended in order to define a certain type of
 * supervisor. You must define the behavior of each supervisor implementing the
 * run function. Only one rule at a time can be active in a group. In each group
 * there is only a supervisor that works with the followers. A group, for exist,
 * must have an active supervisorRole in its members.
 * 
 * @author bett.marco88@gmail.com
 * 
 */

// TODO guardare il comportamento di deleter e split manager

public abstract class A3JGSingleThreadedSupervisorRole extends ReceiverAdapter
		implements Runnable {
	...
	protected A3JGSingleThreadedNode node;
	...
	public A3JGSingleThreadedSupervisorRole(int resourceCost, ThreadManager tm) {
		super();
	...
	/**
	 * You have to use this function to send message, that will be saved over
	 * time, to followers. If days, hours and minutes are equal to 0, the
	 * message is valid for infinite.
	 * 
	 * @param mex
	 *            The A3JGMessage sent to followers.
	 * @param dest
	...
	public boolean changeRoleInGroup(int config) {
		...
		String role = node.getGroupInfo(chan.getClusterName()).getSupervisor()
				.get(config);
	...
	/**
	 * This function return the fitness value of the supervisor. This value is
	 * used for choose the best candidate for be the new supervisor during an
	 * election.
	 * 
	 * @return The Integer value of fitness.
	 */
